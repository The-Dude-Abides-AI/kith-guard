...
- **`block-timeout` disambiguation:** The `block-timeout` disposition covers three distinct scenarios: (a) BLOCK_AND_REWRITE double rewrite timeout (~6s), (b) BLOCK_AND_REWRITE re-score timeout (~6s), and (c) BLOCK-path response when re-scoring has been auto-disabled by the re-score failure rate SLO (>4hr sustained degradation). Consumers investigating `block-timeout` events where no timeout appears in the trace should check the re-score failure rate SLO metric — if re-scoring is auto-disabled, the `block-timeout` indicates an administrative disable rather than a transient timeout.
