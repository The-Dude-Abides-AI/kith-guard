...
**Rewrite error handling:** Non-timeout rewrite failures (rate limit, context-too-long, API error, connection refused) are treated identically to rewrite timeouts — REWRITE path ships the original with `rewrite-timeout`, BLOCK_AND_REWRITE path retries once then ships generic fallback with `block-timeout`. The same applies to non-timeout re-score failures. All rewrite/re-score errors count toward the judge circuit breaker (Decision 2) — if the primary model is failing repeatedly, the gate should degrade gracefully rather than accumulate errors.
