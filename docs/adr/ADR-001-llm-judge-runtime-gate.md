...
**Circuit breaker attribution:** Rewrite errors (Opus calls) count toward a **separate rewrite circuit breaker** (3 **consecutive** failures in 60 seconds → disable rewrites for 5 minutes), independent of the judge circuit breaker in Decision 2. **Re-score errors feed the judge circuit breaker**, not the rewrite CB — re-scoring is a Prometheus evaluation (same rubric, same model as the initial judgment), so re-score failures are evidence of Prometheus/Ollama health degradation, not Opus health degradation. The judge circuit breaker tracks Prometheus/Ollama health (initial judgment + re-score); the rewrite circuit breaker tracks primary model (Opus) health (rewrite generation only). When the rewrite circuit breaker opens, the judge still runs and scores responses, but all rewrite actions degrade: REWRITE-path responses ship the original with `rewrite-cb-open`, BLOCK_AND_REWRITE-path responses ship the generic fallback with `block-cb-open`. These dedicated dispositions distinguish circuit-breaker-induced skips (~2s, judge only) from actual rewrite timeouts (~4–6s, rewrite was attempted). Responses during a rewrite circuit breaker window are NOT excluded from the unjudged-rate SLO (they are judged — only the rewrite is skipped).
