...
| Rewrite timeout — BLOCK_AND_REWRITE path, retry succeeds + re-score timeout (enforcement only) | N/A | N/A | ~2s judge + 2s rewrite timeout + ~2s retry + 2s re-score timeout = **~8s**, ships generic fallback with `block-timeout` (cannot confirm rewrite cleared block threshold) |
...
| Re-score failure rate | > 50% over rolling 1-hour window | Ops alert + rubric review. If sustained > 2 hours: incident escalation. If sustained > 4 hours: auto-disable re-scoring (rewrites ship unconfirmed with `rewrite`, BLOCK path ships generic fallback with `block-timeout`). **Recovery:** 10 consecutive successful re-score calls → auto-re-enable re-scoring + ops alert that re-scoring has resumed. Higher threshold than CB recovery (10 vs 3) because auto-disable is triggered by sustained degradation, not transient failures — requires stronger evidence of recovery before re-enabling. Catches selective Prometheus degradation under re-score load that interleaving masks from the judge CB consecutive-failure counter |
...
- **BLOCK_AND_REWRITE** (score ≥ block_threshold): on timeout, the original response MUST NOT ship (it was blocked for a reason). Instead: **re-check rewrite circuit breaker state before retrying.** If the CB opened in the interval since the first timeout (e.g., this was the 3rd failure that tripped the CB), skip the retry and ship the generic fallback with `X-KithGuard: block-cb-open`. If the CB is still closed, retry the rewrite once with a fresh 2s budget. If the retry also times out, ship a **generic fallback** ("Let me reconsider and follow up") with `X-KithGuard: block-timeout` and log the full original + context for async review. This preserves the block semantic at the cost of a degraded response while respecting the CB's purpose of not hammering a failing service.

**Rewrite error handling:** Non-timeout rewrite failures (rate limit, context-too-long, API error, connection refused) are treated identically to rewrite timeouts — REWRITE path ships the original with `rewrite-timeout`, BLOCK_AND_REWRITE path retries once then ships generic fallback with `block-timeout`. The same applies to non-timeout re-score failures (same timeout behavior per path).

**Circuit breaker attribution:** Rewrite errors (Opus calls) count toward a **separate rewrite circuit breaker** (3 **consecutive** failures in 60 seconds → disable rewrites for 5 minutes), independent of the judge circuit breaker in Decision 2. **Re-score errors feed the judge circuit breaker**, not the rewrite CB — re-scoring is a Prometheus evaluation (same rubric, same model as the initial judgment), so re-score failures are evidence of Prometheus/Ollama health degradation, not Opus health degradation. The judge circuit breaker tracks Prometheus/Ollama health (initial judgment + re-score); the rewrite circuit breaker tracks primary model (Opus) health (rewrite generation only).

**Judge CB consecutive-failure semantics with interleaved calls:** The judge CB counter tracks all Prometheus calls (judgment + re-score) as a single stream. Any successful Prometheus call (whether judgment or re-score) resets the consecutive-failure counter. This means a scenario where judgments succeed but re-scores consistently fail will not trip the judge CB if successful judgments interleave frequently enough. To catch this degradation pattern, a **re-score failure rate SLO** supplements the CB: if re-score failures exceed 50% over a rolling 1-hour window, an ops alert fires regardless of CB state. This ensures that selective Prometheus degradation under re-score load (heavier than single-call judgments) is surfaced even when the CB remains closed. When the rewrite circuit breaker opens, the judge still runs and scores responses, but all rewrite actions degrade: REWRITE-path responses ship the original with `rewrite-cb-open`, BLOCK_AND_REWRITE-path responses ship the generic fallback with `block-cb-open`. These dedicated dispositions distinguish circuit-breaker-induced skips (~2s, judge only) from actual rewrite timeouts (~4–6s, rewrite was attempted). Responses during a rewrite circuit breaker window are NOT excluded from the unjudged-rate SLO (they are judged — only the rewrite is skipped).

The same 2s budget applies to the re-score call (mandatory in enforcement mode). **Re-score timeout behavior:**
...
- **`block-timeout` disambiguation:** The `block-timeout` disposition covers four distinct scenarios: (a) BLOCK_AND_REWRITE double rewrite timeout (~6s), (b) BLOCK_AND_REWRITE direct re-score timeout (~6s: judge → rewrite → re-score timeout), (b2) BLOCK_AND_REWRITE retry-success + re-score timeout (~8s: first rewrite times out, retry succeeds, re-score times out), and (c) BLOCK-path response when re-scoring has been auto-disabled by the re-score failure rate SLO (>4hr sustained degradation). Consumers investigating `block-timeout` events where no timeout appears in the trace should check the re-score failure rate SLO metric — if re-scoring is auto-disabled, the `block-timeout` indicates an administrative disable rather than a transient timeout.
...
- **Latency overhead** — Up to 6s in the common case for flagged+rewritten+re-scored responses; up to ~8s in rare BLOCK_AND_REWRITE retry-success scenarios (see Decision 1 latency table). ~2s on triggered-but-passing responses (mitigated by selective triggering — 85–95% of responses bypass entirely)
...
| v10 | 2026-03-10 | The Dude (Opus 4.6) | Third review pass (9 comments): (1) latency table ~4s enforcement row removed — no valid rewrite-without-rescore path, re-score mandatory, (2) inter-rater kappa AND raw agreement both required with kappa as binding constraint, (3) `rewrite-cb-open` / `block-cb-open` dispositions added to distinguish CB skip from actual timeout, (4) first-response added to Tier 1 override exceptions — capitulation is logically impossible without prior position, (5) `gate_decision` precondition guard + docstring for disabled agents, (6) consumer guidance: check `X-KithGuard-Rescore` presence for confirmed vs unconfirmed rewrites, (7) SOC 2 CC8.1 updated to reference both unjudged-rate SLOs (20% non-CB + 50% all-causes), (8) BLOCK_AND_REWRITE retry-success path defined — ~8s documented exception to 6s ceiling, (9) consumer guidance: check `X-KithGuard-Score` to disambiguate `rewrite-failed` across REWRITE vs BLOCK_AND_REWRITE paths |
| v11 | 2026-03-10 | The Dude (Opus 4.6) | Fourth review pass (6 comments): (1) CB re-check before BLOCK_AND_REWRITE retry — if CB opened since first timeout, skip retry and ship `block-cb-open`, (2) latency table row for retry-success + re-score-timeout (~8s → `block-timeout`), (3) rubric file existence added to startup validation alongside threshold check, (4) `block` disposition always carries rescore — consumer guidance corrected to reference only `rewrite` for unconfirmed path, (5) sequence diagram BLOCK_AND_REWRITE timeout branch updated to show CB re-check and `block-cb-open` outcome, (6) version history v11 |
| v12 | 2026-03-10 | The Dude (Opus 4.6) | Fifth review pass (3 comments): (1) `block-cb-open` bimodal latency — two rows: ~2s (CB already open) and ~4s (first timeout triggers CB), (2) sequence diagram retry-success re-score outcomes enumerated for BLOCK path, (3) REWRITE side of diagram made symmetric — CB check, `rewrite-cb-open` branch, and all re-score outcomes added |
| v13 | 2026-03-12 | The Dude (Opus 4.6) | Sixth review pass (6 comments): (1) "one documented exception" → two ~8s paths, (2) stale "optional" re-score language → "mandatory in enforcement mode", (3) duplicated rationale sentence collapsed, (4) trigger conditions 2 & 3 given explicit phase-dependency notes (Tier 1 regex for Phases 1-2, Tier 2 for Phase 3+), (5) rewrite CB aligned to consecutive-failure semantics (was ambiguous sliding-window), (6) re-score CB attribution corrected — re-score is a Prometheus call, errors now feed judge CB not rewrite CB |
| v14 | 2026-03-12 | The Dude (Opus 4.6) | Seventh review pass (8 comments): (1) rewrite CB monitoring SLO added (30min alert, 60min incident) + recovery condition, (2) re-score failure rate SLO with 3-tier escalation (1hr alert, 2hr incident, 4hr auto-disable) + recovery (10 consecutive successes), (3) judge CB interleaving semantics clarified + re-score failure rate as secondary catch, (4) sequence diagram labeled "Enforcement Mode" with shadow/advisory note, (5) family-violation re-enablement defined as restart-only, (6) Consequences latency updated to include ~8s, (7) Decision 9 cost summary updated to "common worst-case ~6s" with ~8s exceptions, (8) CONSENSAGENT tier label clarified, (9) `block-timeout` consumer guidance expanded to 4 scenarios (added ~8s retry-success + re-score timeout path), (10) re-score auto-disable recovery condition added |
